...
int main(int argc, char *argv[], struct thread_inf)
{
...
/*#pragma omp parallel for num_threads(p)
    ...
    for(int i=0; i<allChange.size(); i=i+batch)
    {
        ...
        if(checkx1 >= timeallowed)
        {
            ...
            //extern "C" void SIMPLE_main(&X, nodes, p, THREADED);
            Boruvka_sparse_merge(&X, nodes, TH); //not passing 'p', e.g., number of threads
