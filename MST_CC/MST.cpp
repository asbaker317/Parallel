...
vector <int_double> the_list[52]={{11,1},  {11,1},  {546162,1},  {43447,1},  {200119,1},  {567943,1},  {188711,1},  {11,1},  {11,1},
...
void *SIMPLE_main(THREADED)
{
  ...
  if(MYTHREAD==0){
	t = read_graph(input_file,&n_vertices,&d_or_s,&Y,&grph_matrix);
	if(t!=0) exit(0);
  }
  ...
  if(d_or_s==DENSE) {
	...
	on_one_thread{
		for(i=0;i<n_vertices;i++){
			if(Y->at(i).ListW.size()>max_d) max_d=Y->at(i).ListW.size();
			if(Y->at(i).ListW.size()<min_d) min_d=Y->at(i).ListW.size();
			}
...
#if 1
	start=gethrtime();
	Boruvka_sparse_merge(Y,n_vertices,TH); ///////////////////////////////
 	node_Barrier();
  	end = gethrtime();
  	interval=end-start;
